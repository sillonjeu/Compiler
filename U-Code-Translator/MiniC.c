...
//	if (!mainExist) warningmsg("main does not exist");
	...
	// generate codes for start routine Ucode 
	//          bgn    globalSize
	//			ldp
    //          call    main
	//          end
...
//	dumpSymbolTable();
	//genSym(base);
	for (stIdx = levelTable[top - 1]; stIdx < avail; stIdx++)
	{
		if ((symbolTable[stIdx].typeQualifier == CONST_TYPE) || (symbolTable[stIdx].typeQualifier == FUNC_TYPE))
			continue;
		emit3(sym, 2, symbolTable[stIdx].offset, symbolTable[stIdx].width);
	}
